...
namespace ConsoleQuest
{
    class Program
    {
        ...
        static int currentHealth = 60; // 현재 체력
        static int maxHealth = 100;     // 최대 체력
        static int currentGold = 1000;     // 현재 가지고 있는 돈
        ...
        static void ProcessInput()
        {
            ...
            switch (keyInfo.Key)
            {
                ...
                case ConsoleKey.UpArrow:
                    nextY = playerYPosition - 1; // 0,0이 좌상단 y값은 아래로갈수록 증가한다 ! 고로 이게 맞다 !
                    break;
                case ConsoleKey.DownArrow:
                    nextY = playerYPosition + 1;
                    break;
        ...
        static void StartBattle()
        {
            ...
            {    
                case 0:
                monsterName = "슬라임";
                monsterDamage = random.Next(5, 10);
                monsterReward = random.Next(5, 15);
                break;

                case 1:
                monsterName = "고블린";
                monsterDamage = random.Next(7, 13);
                monsterReward = random.Next(10, 20);
                break;

                case 2:
                monsterName = "오크";
                monsterDamage = random.Next(15, 20);
                monsterReward = random.Next(30, 40);
                break;
            }
        ...
        static void EnterShop()
        {
            ...
            while (inShop)
            {
                ...
                else if (input == "1") // 빨간 물약
                {
                    ...
                    if(currentGold < price)  // 보유 금화 체크.
                    {
                        Console.WriteLine("금화가 부족합니다.");
                        System.Threading.Thread.Sleep(1000);
                    }

                    else if(currentHealth == maxHealth) // 체력이 가득차 있을때 체크
                    {
                        Console.WriteLine("체력이 가득 차있습니다.");
                        System.Threading.Thread.Sleep(1000);
                    }
                    ...
                    else // 정상 구매
                    {
                        currentHealth += 20;
                        currentGold -= price;
                        Console.WriteLine("체력을 20 회복했습니다.");
                        System.Threading.Thread.Sleep(1000);

                        if(currentHealth > maxHealth) currentHealth = maxHealth; // 현재 체력이 최대 체력을 넘어가지 않게 처리
                    }
                    // ---------------------------------------------------------
                    // [과제 2] 구매 로직 구현하기
                    // ---------------------------------------------------------
                    // 가격체크 - 돈 충분 - 체력회복(최대체력보다 높아진다면 최대체력으로.) - 돈 차감 
                    //        ㄴ 돈 부족 - 돈 부족 알림
                ...
                else if (input == "2") // 갑옷
                {
                    ...
                    else // 정상 구매
                    {
                        ...
                        currentHealth +=20;
                        currentGold -= price;
                        Console.WriteLine("갑옷을 구매했습니다. 최대 체력 + 20");
                        System.Threading.Thread.Sleep(1000);
                    }
                ...
                else if (input == "3") // 검
                {
                    ...
                    else // 정상 구매
                    {
                        attackPower += 5;
                        currentGold -= price;
                        Console.WriteLine("검을 구매했습니다. 공격력 + 5");
                        System.Threading.Thread.Sleep(1000);
                    }
                }
                else
